(() => {
  ...
  const GAME_VERSION = 'Alpha 2.0b.2';
  const RULES_VERSION = '0.3-D';
  ...
  let sb = null;
  let state = {session:null,teacherProfile:null,room:null,me:null,players:[],match:null,privateStates:[],channel:null,busy:false,dashboard:null,historyDetail:null,resumeSnapshot:null,interrupting:false,deferTeacherRender:false};
  ...
  function renderHome(){
    stopRoomWatch();state.room=null;state.me=null;state.players=[];state.match=null;state.privateStates=[];state.historyDetail=null;state.resumeSnapshot=null;setConn(navigator.onLine?'Conectado':'Sem internet',navigator.onLine?'ok':'bad');
    const teacherLogged=Boolean(state.session&&!sessionIsAnonymous()&&state.teacherProfile);
    screen.innerHTML=`<div class="card hero"><div class="eyebrow">GeoPoder · ${GAME_VERSION}</div><h2>Entre na partida</h2><p class="muted">Quatro equipes jogam em aparelhos próprios. O acesso do professor é autenticado.</p></div><div class="grid two" style="margin-top:16px"><section class="card role-card"><h3>🌍 Equipe</h3><div class="field"><label>Código da sala</label><input id="roomCode" maxlength="8" placeholder="GEO-AB12" style="text-transform:uppercase"></div><div class="field"><label>Nome da equipe</label><input id="teamName" maxlength="60" placeholder="Ex.: Equipe Global"></div><div class="field"><label>País</label><select id="country">${COUNTRIES.map(c=>`<option>${c}</option>`).join('')}</select></div><div class="actions"><button class="btn good" id="joinRoom">Entrar na sala</button></div></section><section class="card role-card teacher-entry"><div class="eyebrow">Acesso restrito</div><h3>🎓 Área do Professor</h3><p class="muted">Criação de salas, histórico, telemetria e controles do playtest.</p><div class="actions"><button class="btn violet" id="teacherEntry">${teacherLogged?'Abrir painel':'Entrar como professor'}</button></div></section></div>`;
    document.getElementById('joinRoom').onclick=()=>withBusy(async()=>{const code=document.getElementById('roomCode').value.trim().toUpperCase(),teamName=document.getElementById('teamName').value.trim(),country=document.getElementById('country').value;if(!code||!teamName)throw new Error('Informe o código e o nome da equipe.');await ensureAnonymousSession();const d=await api('join_room',{code,teamName,country});rememberRoom(d.room.id);await loadRoom(d.room.id)});
  ...
  function renderHistoryDetail(){const d=state.historyDetail;if(!d)return loadTeacherDashboard();const teams=(d.players||[]).filter(p=>p.role==='player'),events=d.events||[],counts=countEvents(events),q=questionStats(events),c=cardStats(events),start=d.match?.started_at||d.room.started_at,end=d.match?.ended_at||d.room.ended_at,dur=d.match?.duration_seconds??(start&&end?Math.round((new Date(end)-new Date(start))/1000):null);screen.innerHTML=`<section class="card"><div class="dashboard-head"><div><div class="eyebrow">Relatório de partida</div><h2>${esc(d.room.class_name||d.room.code)}</h2><div class="statusline">${statusBadge(d.room.status)}<span class="badge">${esc(d.room.code)}</span><span class="badge">${esc(d.match?.game_version||'—')}</span></div></div><div class="actions"><button class="btn ghost" id="historyBack">Voltar</button><button class="btn ghost" id="historyMd">Relatório (.md)</button><button class="btn ghost" id="historyCsv">Planilha (.csv)</button><button class="btn ghost" id="historyJson">Dados completos (.json)</button></div></div></section><section class="grid four" style="margin-top:16px"><div class="metric"><div class="value">${teams.length}</div><div class="label">equipes</div></div><div class="metric"><div class="value">${d.match?.round??'—'}</div><div class="label">última rodada</div></div><div class="metric"><div class="value">${fmtDuration(dur)}</div><div class="label">duração</div></div><div class="metric"><div class="value">${events.length}</div><div class="label">eventos</div></div></section><section class="grid two" style="margin-top:16px"><div class="card"><div class="eyebrow">Aprendizagem</div><h3>Desafios</h3>${q.total?`<div class="metric"><div class="value">${q.correct}/${q.total}</div><div class="label">acertos (${Math.round(q.correct/q.total*100)}%)</div></div>`:'<p class="muted">Sem respostas registradas.</p>'}</div><div class="card"><div class="eyebrow">Playtest</div><h3>Cartas</h3><p class="muted">Compradas: ${Object.values(c.drawn).reduce((a,b)=>a+b,0)} · Jogadas: ${Object.values(c.played).reduce((a,b)=>a+b,0)} · Descartadas: ${Object.values(c.discarded).reduce((a,b)=>a+b,0)}</p></div></section><section class="card" style="margin-top:16px"><div class="eyebrow">Telemetria</div><h3>Eventos registrados</h3><div class="table-wrap"><table><thead><tr><th>Tipo</th><th>Quantidade</th></tr></thead><tbody>${Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<tr><td>${esc(k)}</td><td>${v}</td></tr>`).join('')}</tbody></table></div></section>`;document.getElementById('historyBack').onclick=loadTeacherDashboard;document.getElementById('historyMd').onclick=()=>exportDetail('md');document.getElementById('historyCsv').onclick=()=>exportDetail('csv');document.getElementById('historyJson').onclick=()=>exportDetail('json')}
  ...
  function renderRoom(){if(!state.room||!state.me)return renderHome();state.match?renderMatch(state.me.role==='teacher'):renderLobby(state.me.role==='teacher')}
  ...
  function renderLobby(isTeacher){const roster=rosterByCountry(),count=COUNTRIES.filter(c=>roster[c]).length;screen.innerHTML=`<section class="card"><div class="statusline"><span class="badge">Sala</span><span class="badge warn">Lobby</span>${state.room.class_name?`<span class="badge">${esc(state.room.class_name)}</span>`:''}</div><div class="room-code">${esc(state.room.code)}</div><div class="copyline"><span class="muted">Compartilhe este código.</span><button class="btn ghost" id="copyCode">Copiar</button><button class="btn ghost right" id="leaveRoom">${isTeacher?'Voltar ao painel':'Sair / entrar em outra sala'}</button></div></section><section class="card" style="margin-top:16px"><div class="actions"><div><h3>Equipes</h3><div class="muted small">${count}/4 países ocupados</div></div>${isTeacher?`<button class="btn primary right" id="startMatch" ${count===4?'':'disabled'}>Iniciar partida</button>`:''}</div><div class="grid four" style="margin-top:16px">${COUNTRIES.map(c=>seatHtml(c,roster[c])).join('')}</div></section>`;document.getElementById('copyCode').onclick=()=>navigator.clipboard?.writeText(state.room.code);document.getElementById('leaveRoom').onclick=()=>leaveCurrentRoom(isTeacher);if(isTeacher&&count===4)document.getElementById('startMatch').onclick=()=>withBusy(async()=>{await api('start_match',{roomId:state.room.id});await refreshSnapshot()})}
  ...
  function renderMatch(isTeacher){
    ...
    screen.innerHTML=`<section class="card game-header"><div class="dashboard-head"><div><div class="eyebrow">Sala ${esc(state.room.code)} · Rodada ${pub.round}/8</div><h2 style="margin:.15rem 0">${phaseTitle(pub)}</h2><div class="version-line">${esc(pub.game_version||GAME_VERSION)} · regras ${esc(pub.rules_version||RULES_VERSION)}</div></div><div class="actions"><span class="badge violet">${esc(pub.phase)}</span>${pub.deadline?`<span class="timer" data-deadline="${esc(pub.deadline)}">—</span>`:''}<button class="btn ghost" id="leaveActive">${isTeacher?'Painel':'Sair / outra sala'}</button></div></div>${pub.current_event?eventBanner(pub.current_event):''}${renderRecentLog(pub.recent_log)}</section><section class="grid four" style="margin-top:16px">${COUNTRIES.map(c=>countryHtml(c,pub.countries?.[c]||{},c===myCountry,pub)).join('')}</section>${renderPhasePanel(pub,isTeacher,own)}${!isTeacher?renderOwnHand(own):renderTeacherPanel(pub)}${isTeacher?teacherTelemetryHtml():''}`;
    document.getElementById('leaveActive').onclick=()=>leaveCurrentRoom(isTeacher);bindMatchActions(pub,isTeacher,own);updateCountdowns();
  }
  function phaseTitle(pub){if(pub.pending_public?.kind==='renew_discard')return'♻ Renovação de carta';if(pub.pending_public?.kind==='hand_limit_discard'||pub.pending_public?.kind==='discard_card')return'🗑️ Escolha de descarte';if(pub.phase==='event')return'🌍 Evento Mundial';if(pub.phase==='challenge')return'🧠 Desafio Geográfico';if(pub.phase==='challenge_result')return'✅ Resultado do Desafio';if(pub.phase==='draw')return'🃏 Compra e renovação';if(pub.phase==='turn_ready')return`⏳ ${pub.active_country} se prepara`;if(pub.phase==='turns')return`Turno de ${pub.active_country}`;return'Partida em andamento'}
  function eventBanner(e){return `<div class="event-banner"><div><div class="eyebrow">${esc(e.kind)}</div><h3>${esc(e.name)}</h3><p>${esc(e.effect)}</p></div></div>`}
  function renderRecentLog(log){if(!log?.length)return'';return `<div class="turn-feed"><div class="eyebrow">Últimos resultados</div>${log.slice(-4).map(x=>`<div>${esc(x.text||x)}</div>`).join('')}</div>`}

  function renderPhasePanel(pub,isTeacher,own){
    // Pending decisions have visual priority over the phase summary. This is
    // essential for leader renewals created immediately after a Challenge.
    ...
    if(pub.phase==='challenge_result')return renderChallengeResult(pub);
    if(pub.phase==='turn_ready')return renderTurnReady(pub,isTeacher);
    if(pub.phase==='turns')return renderTurn(pub,isTeacher);
    if(pub.phase==='draw'||pub.phase==='event')return `<section class="card action-panel" style="margin-top:16px"><div class="waiting-orb"></div><h3>Resolução automática</h3><p class="muted">Aguardando a próxima decisão necessária.</p></section>`;
    return'';
  }

  function renderChallenge(pub,isTeacher){const q=pub.current_challenge,answered=pub.challenge_answered||[],mine=state.me.country,has=answered.includes(mine);return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Todos respondem ao mesmo tempo</div><h3>${esc(q.question)}</h3>${isTeacher?`<div class="answer-status">${COUNTRIES.map(c=>`<span class="badge ${answered.includes(c)?'good':''}">${c}: ${answered.includes(c)?'✓':'…'}</span>`).join('')}</div>`:has?'<div class="banner good">Resposta registrada. Aguarde as outras equipes.</div>':`<div class="option-grid">${q.options.map((o,i)=>`<button class="option-btn" data-challenge-answer="${i}"><b>${String.fromCharCode(65+i)}</b><span>${esc(o)}</span></button>`).join('')}</div>`}</section>`}
  function renderChallengeResult(pub){const r=pub.challenge_result,q=pub.current_challenge;if(!r)return'';return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Resposta correta</div><h3>${String.fromCharCode(65+r.correct)} — ${esc(q.options[r.correct])}</h3><div class="grid four result-grid">${r.results.map(x=>`<div class="result-tile ${x.correct?'correct':'wrong'}"><b>${esc(x.country)}</b><span>${x.answer==null?'Sem resposta':`${String.fromCharCode(65+x.answer)} · ${x.correct?'Acertou':'Errou'}`}</span></div>`).join('')}</div></section>`}

  function renderPending(p,isTeacher,own){
    const mine=state.me.country,forMe=!isTeacher&&p.country===mine;
    if(!forMe)return `<section class="card action-panel" style="margin-top:16px"><div class="waiting-orb"></div><div class="eyebrow">Aguardando decisão</div><h3>${esc(p.country||'Sistema')} está resolvendo uma escolha</h3><p class="muted">${pendingDescription(p)}</p></section>`;
    if(p.kind==='event_choice')return eventChoicePanel(p);
    if(p.kind==='agreement_response')return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Proposta diplomática</div><h3>${esc(p.proposer)} propõe um Acordo Comercial</h3><p>Aceitar cria uma relação comercial e ambos renovam uma carta da mão.</p><div class="actions"><button class="btn good" data-pending="accept">Aceitar</button><button class="btn danger" data-pending="decline">Recusar</button></div></section>`;
    if(p.kind==='loss_choice')return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Perda de atributo</div><h3>${esc(p.source)}</h3><p>Você perderá <b>${p.amount}</b> em ${ATTRS[p.attr]||p.attr}.</p><div class="actions"><button class="btn danger" data-pending="accept">Sofrer perda</button>${p.can_use_advantage?'<button class="btn violet" data-pending="use_advantage">🎓 Gastar Vantagem e reduzir 1</button>':''}</div></section>`;
    if(p.kind==='redirect_loss')return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Atributo zerado</div><h3>Redirecione ${p.remaining} ponto(s) de perda</h3><p class="muted">A perda não pôde ser aplicada integralmente em ${ATTRS[p.original_attr]||p.original_attr}. Escolha outro atributo.</p><div class="actions">${(p.available_attrs||[]).map(a=>`<button class="btn" data-pending="${a}">${ATTRS[a]}</button>`).join('')}</div></section>`;
    if(p.kind==='event_roll_decision')return `<section class="card action-panel dice-panel" style="margin-top:16px"><div class="eyebrow">Rolagem pública</div><div class="big-die">🎲 ${p.roll}</div><h3>${esc(p.source)}</h3><div class="actions"><button class="btn primary" data-pending="accept">Aceitar resultado</button>${p.can_reroll?'<button class="btn violet" data-pending="reroll">🎓 Gastar Vantagem e rerrolar</button>':''}</div></section>`;
    if(['discard_card','renew_discard','hand_limit_discard'].includes(p.kind))return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Escolha uma carta</div><h3>${esc(p.reason||'Descarte')}</h3><p class="muted">Leia o efeito antes de decidir.</p>${handHtml(own?.hand||[],{buttonFor:id=>({button:'Descartar',data:`data-pending="${id}"`})})}</section>`;
    if(p.kind==='draw_choice')return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Compra da rodada</div><h3>Como deseja comprar?</h3><div class="actions"><button class="btn primary" data-pending="normal">Comprar 1 carta</button><button class="btn violet" data-pending="advantage">🎓 Gastar Vantagem: ver 2 e ficar com 1</button></div></section>`;
    if(p.kind==='draw_keep'){const ids=own?.pending?.candidates||[];return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Vantagem Geográfica</div><h3>Escolha qual carta ficará na sua mão</h3><div class="hand">${ids.map(id=>cardHtml(id,{button:'Ficar com esta',data:`data-pending="${id}"`})).join('')}</div></section>`}
    return `<section class="card action-panel" style="margin-top:16px"><h3>Decisão necessária</h3><p>${esc(pendingDescription(p))}</p></section>`;
  }
  function pendingDescription(p){return({event_choice:'Escolha do Evento Mundial.',agreement_response:'Resposta a uma proposta comercial.',loss_choice:'Decisão sobre uma perda.',redirect_loss:'Redirecionamento de perda.',event_roll_decision:'Decisão sobre uma rolagem.',renew_discard:'Renovação de carta.',hand_limit_discard:'Descarte por limite da mão.',draw_choice:'Compra da rodada.',draw_keep:'Escolha entre duas cartas.'})[p.kind]||p.kind}
  function eventChoicePanel(p){
    ...
    if(eid===4)html='<button class="btn danger" data-pending="lose">Perder 1 Diplomacia</button><button class="btn" data-pending="block_diplomacy">Não usar Diplomacia nesta rodada</button>';
  ...
  function renderTurnReady(pub,isTeacher){
    ...
    if(isTeacher)return `<section class="card action-panel turn-ready" style="margin-top:16px"><div class="eyebrow">Turno aguardando início</div><h3>${esc(active)} ainda não iniciou o cronômetro</h3><p class="muted">Use este intervalo para orientar a turma. O tempo de 45 s só começa após a confirmação da equipe.</p><div class="actions"><button class="btn primary" id="teacherStartTurn">Iniciar turno agora</button></div></section>`;
    if(state.me.country===active)return `<section class="card action-panel active-turn turn-ready" style="margin-top:16px"><div class="eyebrow">Sua vez</div><h3>Está tudo pronto?</h3><p>Confira seus atributos e sua mão. O cronômetro <b>ainda não começou</b>.</p><div class="actions"><button class="btn primary ready-btn" id="playerReadyTurn">Estamos prontos — começar turno</button></div></section>`;
    return `<section class="card action-panel" style="margin-top:16px"><div class="waiting-orb"></div><div class="eyebrow">Preparação do turno</div><h3>Aguardando ${esc(active)} confirmar que está pronto</h3><p class="muted">O cronômetro ainda não está correndo.</p></section>`;
  }

  function renderTurn(pub,isTeacher){if(isTeacher)return `<section class="card action-panel" style="margin-top:16px"><div class="eyebrow">Turno ativo</div><h3>${esc(pub.active_country)}</h3><p class="muted">O jogador pode Passar ou usar Recuperação Nacional. As 28 cartas serão conectadas ao motor no próximo incremento da 2.0b.</p></section>`;if(state.me.country!==pub.active_country)return `<section class="card action-panel" style="margin-top:16px"><div class="waiting-orb"></div><h3>Turno de ${esc(pub.active_country)}</h3><p class="muted">Acompanhe os resultados acima. Sua tela será liberada automaticamente quando chegar sua vez.</p></section>`;const c=pub.countries[state.me.country];const zeros=Object.keys(ATTRS).filter(a=>Number(c[a])===0);return `<section class="card action-panel active-turn" style="margin-top:16px"><div class="eyebrow">Sua Ação Principal</div><h3>Decida antes do tempo acabar</h3><div class="actions"><button class="btn primary" data-turn-action="pass">Passar vez</button>${zeros.map(a=>`<button class="btn good" data-recovery="${a}">Recuperação: ${ATTRS[a]} 0 → 1</button>`).join('')}</div><p class="muted small">Recuperação Nacional só pode elevar um atributo que começou a ação em 0 para 1.</p></section>`}
  function renderOwnHand(own){return `<section class="card" style="margin-top:16px"><div class="eyebrow">Sua mão privada</div><h3>Cartas</h3>${handHtml(own?.hand||[],{buttonFor:id=>({button:'Disponível em breve',disabled:true})})}</section>`}
  function renderTeacherPanel(pub){return `<section class="card" style="margin-top:16px"><div class="dashboard-head"><div><div class="eyebrow">Controle de playtest</div><h3>Professor</h3></div><div class="actions">${pub.deadline?'<button class="btn ghost" id="teacherAddTime">+30 s</button>':''}${pub.phase==='turns'&&!pub.pending_public?'<button class="btn danger" id="teacherForcePass">Forçar passar vez</button>':''}<button class="btn danger" id="interruptSession">Interromper sessão</button></div></div><div class="banner warn">Cartas ainda não executam efeitos nesta entrega. A 2.0b.1 estabiliza o motor, o cronômetro e a condução em sala.</div></section>`}
  function teacherTelemetryHtml(){const draft=esc(getNoteDraft());return `<section class="card" style="margin-top:16px"><div class="eyebrow">Observação</div><h3>Nota de playtest</h3><div class="field"><textarea id="teacherNote" placeholder="Registre comportamento, dúvida ou problema observado. O rascunho é preservado automaticamente.">${draft}</textarea></div><div class="actions"><button class="btn" id="saveNote">Salvar nota</button><button class="btn ghost" id="exportMd">Relatório (.md)</button><button class="btn ghost" id="exportCsv">Planilha (.csv)</button><button class="btn ghost" id="exportJson">Dados completos (.json)</button></div><p class="muted tiny">Enquanto você estiver escrevendo, o painel evita reconstruir esta caixa. O rascunho também fica salvo no navegador até ser enviado.</p><div id="noteFeedback"></div></section>`}
  ...
  function bindMatchActions(pub,isTeacher,own){
    ...
    if(isTeacher){
      document.getElementById('teacherAddTime')?.addEventListener('click',()=>withBusy(async()=>{await api('teacher_add_time',{roomId:state.room.id});await refreshSnapshot()}));
      ...
      document.getElementById('teacherForcePass')?.addEventListener('click',()=>withBusy(async()=>{await api('teacher_force_pass',{roomId:state.room.id});await refreshSnapshot()}));
  ...
  function buildMarkdown(d){const teams=(d.players||[]).filter(p=>p.role==='player'),events=d.events||[],counts=countEvents(events),q=questionStats(events);let md=`# GeoPoder — Relatório de Playtest\n\n- **Sala:** ${d.room.code}\n- **Turma:** ${d.room.class_name||'—'}\n- **Status:** ${statusLabel(d.room.status)}\n- **Versão:** ${d.match?.game_version||'—'}\n- **Regras:** ${d.match?.rules_version||'—'}\n\n## Equipes\n`;for(const c of COUNTRIES){const p=teams.find(x=>x.country===c);md+=`- **${c}:** ${p?.team_name||'—'}\n`}md+=`\n## Aprendizagem\n- Respostas: ${q.total}\n- Acertos: ${q.correct}\n- Erros: ${q.wrong}\n${q.total?`- Percentual: ${Math.round(q.correct/q.total*100)}%\n`:''}\n## Telemetria\n`;for(const[k,v]of Object.entries(counts).sort((a,b)=>b[1]-a[1]))md+=`- ${k}: ${v}\n`;md+='\n## Eventos brutos\n\n```json\n'+JSON.stringify(events,null,2)+'\n```\n';return md}
