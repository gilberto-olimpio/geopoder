(() => {
  ...
  const CARDS = [
    ...
    C(11,'Barreiras Tarifárias','Interferência',['Acordo'],'Escolha parceiro de Acordo. O alvo escolhe: absorver o impacto (-1 Economia) OU retaliar (atacante -1 Economia, alvo -1 Diplomacia e o Acordo fica sob Tensão até ser reparado na Cúpula).'),
    ...
    C(14,'Pressão Geopolítica','Interferência',['Acordo'],'O alvo escolhe: -1 Diplomacia; descartar 1 carta aleatória; ou, se possuir Acordo utilizável com você, colocar esse Acordo sob Tensão até ser reparado na Cúpula.'),
    ...
    C(16,'Tensão no Bloco','Interferência',['Bloco'],'Escolha um Bloco do qual você não participa. Os membros escolhem: um deles perde 1 Diplomacia OU o Bloco entra em Crise até ser reparado na Cúpula.'),
    C(17,'Embargo Secundário','Interferência',['Acordo'],'Alvo com 2 Acordos utilizáveis escolhe: -1 Economia OU suspender 1 Acordo até ser reparado na Cúpula.'),
    C(18,'Mediação Internacional','Reação',['Defesa'],'Quando uma Interferência causar perda de atributo a você, reduza uma perda em 1.'),
    C(19,'Retaliação Comercial','Reação',['Acordo','Economia'],'Quando uma Interferência fizer você perder Economia: reduza a perda em 1; o atacante perde 1 Economia; se houver Acordo entre vocês, ele fica sob Tensão até ser reparado na Cúpula.'),
  ...
  function mountOnboarding(isTeacher){
    ...
    const steps=[
      {target:'.command-situation',kicker:'BEM-VINDO AO GEOPODER',title:'Seu objetivo é ampliar a Influência do país',text:`A partida dura ${Number(pub.max_rounds||8)} rodadas. Sua Influência é a soma de Economia, Redes, Diplomacia e Cultura. Ao final, quem tiver maior Influência vence; manter todos os quatro atributos em pelo menos 2 concede +2 de Potência Equilibrada.`,button:'Entendi o objetivo'},
  ...
  function renderHome(){
    ...
    stopRoomWatch();state.room=null;state.me=null;state.players=[];state.match=null;state.privateStates=[];state.historyDetail=null;state.resumeSnapshot=null;state.roomPreview=null;state.dismissedSpotlightId=null;state.dismissedGovernmentPostId=null;setConn(navigator.onLine?'Conectado':'Sem internet',navigator.onLine?'ok':'bad');
  ...
  function sessionsTable(items,activeMode){return `<div class="table-wrap"><table><thead><tr><th>Data</th><th>Turma</th><th>Sala</th><th>Status</th><th>Equipes</th><th>Rodada</th><th>Modo</th><th>Versão</th><th></th></tr></thead><tbody>${items.map(s=>`<tr><td>${esc(fmtDate(s.room.created_at))}</td><td>${esc(s.room.class_name||'—')}</td><td class="mono">${esc(s.room.code)}</td><td>${statusBadge(s.room.status)}</td><td>${s.teams?.length||0}</td><td>${s.match?.round??'—'}${s.match?.max_rounds?`/${s.match.max_rounds}`:''}</td><td>${esc(s.match?.match_mode_label||'—')}</td><td>${esc(s.match?.game_version||'—')}</td><td><button class="btn compact ghost" data-open-room="${esc(s.room.id)}">${activeMode?'Abrir':'Relatório'}</button></td></tr>`).join('')}</tbody></table></div>`}
  ...
  function diplomacyStats(events){
    ...
    return{
      ...
      repairs:events.filter(e=>e.event_type==='RELATION_STATE_REPAIRED').length,
  ...
  function turnTimingStats(events){
    ...
    const median=a=>{if(!a.length)return null;const s=[...a].sort((x,y)=>x-y),m=Math.floor(s.length/2);return Math.round(s.length%2?s[m]:(s[m-1]+s[m])/2)};
    ...
    return{turns:turns.length,avgChoice:avg(choice),avgResolution:avg(resolution),avgTotal:avg(total),medianChoice:median(choice),medianResolution:median(resolution),medianTotal:median(total),timeouts:turns.filter(e=>e.payload?.reason==='timeout').length};
  }
  function labStats(events){
    const started=events.find(e=>e.event_type==='MATCH_STARTED')?.payload||{};
    const turns=events.filter(e=>e.event_type==='TURN_FINISHED');
    const passActions=events.filter(e=>e.event_type==='ACTION_COMMITTED'&&e.payload?.action_type==='pass');
    const recoveries=events.filter(e=>e.event_type==='RECOVERY_USED').length;
    const turnPasses=turns.filter(e=>e.payload?.action_type==='pass'||e.payload?.reason==='pass').length;
    const voluntaryPasses=passActions.filter(e=>e.payload?.pass_reason==='voluntary').length;
    const noOptionPasses=passActions.filter(e=>e.payload?.pass_reason==='no_playable_dossier').length;
    const noPlayableTurns=events.filter(e=>e.event_type==='NO_PLAYABLE_DOSSIER').length;
    const replacements=events.filter(e=>e.event_type==='DOSSIER_REPLACED_NO_TARGET').length;
    const summitRounds=new Set(events.filter(e=>e.event_type==='DIPLOMACY_PHASE_STARTED').map(e=>Number(e.round)).filter(Number.isFinite));
    const initiatives=events.filter(e=>e.event_type==='DIPLOMACY_INITIATIVE');
    let summitsNoProposal=0;
  ...
  function renderLobby(isTeacher){
    const roster=rosterByCountry(),count=COUNTRIES.filter(c=>roster[c]).length;
    const cfg=state.lobbyConfig||{matchMode:'complete',initialModel:'digital'};
    const config=isTeacher?`<div class="match-config"><div><label for="matchMode">Duração da partida</label><select id="matchMode"><option value="complete" ${cfg.matchMode==='complete'?'selected':''}>Completa · 8 rodadas</option><option value="short" ${cfg.matchMode==='short'?'selected':''}>Sala de aula · 6 rodadas</option></select><small>A partida curta mantém todas as regras e realiza 3 Desafios.</small></div><div><label for="initialModel">Modelo inicial</label><select id="initialModel"><option value="digital" ${cfg.initialModel==='digital'?'selected':''}>Digital 0.6 · 3–2–1–1</option><option value="legacy" ${cfg.initialModel==='legacy'?'selected':''}>Comparação 0.4-C · 3–2–2–0</option></select><small>O modelo digital evita uma Recuperação obrigatória na abertura.</small></div></div>`:'';
    screen.innerHTML=`<section class="card"><div class="statusline"><span class="badge">Sala</span><span class="badge warn">Lobby</span>${state.room.class_name?`<span class="badge">${esc(state.room.class_name)}</span>`:''}</div><div class="room-code">${esc(state.room.code)}</div><div class="copyline"><span class="muted">Compartilhe este código.</span><button class="btn ghost" id="copyCode">Copiar</button><button class="btn ghost right" id="leaveRoom">${isTeacher?'Voltar ao painel':'Sair / entrar em outra sala'}</button></div></section><section class="card" style="margin-top:16px"><div class="actions"><div><h3>Equipes</h3><div class="muted small">${count}/4 países ocupados · mínimo de 3 para iniciar</div></div>${isTeacher?`<button class="btn primary right" id="startMatch" ${count>=3?'':'disabled'}>Iniciar partida</button>`:''}</div><div class="grid four" style="margin-top:16px">${COUNTRIES.map(c=>seatHtml(c,roster[c])).join('')}</div>${count===3?'<div class="banner good" style="margin-top:14px">Partida com 3 equipes pronta para iniciar. O país vazio ficará fora desta sessão.</div>':''}${config}</section>`;
    document.getElementById('copyCode').onclick=()=>navigator.clipboard?.writeText(state.room.code);
    document.getElementById('leaveRoom').onclick=()=>leaveCurrentRoom(isTeacher);
    document.getElementById('matchMode')?.addEventListener('change',e=>{state.lobbyConfig.matchMode=e.target.value});
    document.getElementById('initialModel')?.addEventListener('change',e=>{state.lobbyConfig.initialModel=e.target.value});
    if(isTeacher&&count>=3)document.getElementById('startMatch').onclick=()=>withBusy(async()=>{
      const matchMode=state.lobbyConfig.matchMode||'complete';
      const initialModel=state.lobbyConfig.initialModel||'digital';
      await api('start_match',{roomId:state.room.id,matchMode,initialModel});await refreshSnapshot();
    });
  }
  ...
  async function leaveCurrentRoom(isTeacher){await withBusy(async()=>{try{await api('leave_room',{roomId:state.room.id})}catch{}state.projectorMode=false;document.body.classList.remove('projector-active');rememberRoom(null);stopRoomWatch();if(isTeacher)await loadTeacherDashboard();else renderHome()})}
  ...
  function cardHasValidTarget(id,pub,country){const act=activeList(pub),me=pub.countries?.[country]||{};if([18,19,20,21,22,23].includes(id))return false;if(id===11)return Object.entries(pub.relations||{}).some(([k,r])=>r.type==='agreement'&&k.split('|').includes(country));if(id===12)return act.some(c=>c!==country&&Number(pub.countries[c]?.eco)>Number(me.eco));if(id===13){const block=publicBlockPartner(pub,country);return act.some(c=>c!==country&&c!==block&&Number(pub.countries[c]?.cult)>=Number(me.cult));}if(id===15)return act.some(c=>c!==country&&Number(pub.countries[c]?.net)>=Number(me.net));if(id===16)return Object.entries(pub.relations||{}).some(([k,r])=>r.type==='block'&&!k.split('|').includes(country));if(id===17)return act.some(c=>c!==country&&publicAgreementPartners(pub,c,{usable:true}).length>=2);return true}
  ...
  function decisionCardSummary(id){
    const c=CARD[Number(id)]||{id:Number(id),name:`Carta ${id}`,type:'',tags:[],effect:''};
    return `<article class="decision-card-summary ${cardTypeClass(c.type)}"><div><span>${esc(cardTypeLabel(c.type))}</span><b>DOSSIÊ #${String(c.id||id).padStart(2,'0')}</b></div><h4>${esc(c.name)}</h4><section><b>EFEITO NO JOGO</b><p>${esc(c.effect)}</p></section></article>`;
  }

  function choiceHandHtml(hand,{button='Escolher esta carta'}={}){
    const ids=hand||[],cols=ids.length>4?3:Math.max(1,ids.length);
    return `<div class="choice-card-grid ${ids.length>4?'many-cards':''}" style="--choice-cols:${cols}">${ids.map(id=>{const c=CARD[Number(id)]||{id:Number(id),name:`Carta ${id}`,type:'',effect:''};return `<article class="choice-card ${cardTypeClass(c.type)}"><div><span>${esc(c.type||'Dossiê')}</span><b>#${String(c.id||id).padStart(2,'0')}</b></div><h4>${esc(c.name)}</h4><p>${esc(c.effect)}</p><button class="btn primary" data-pending="${Number(id)}">${esc(button)}</button></article>`}).join('')||'<p class="muted">Nenhuma carta disponível.</p>'}</div>`;
  }
  ...
  function renderMatch(isTeacher){
    ...
    if(isTeacher&&state.projectorMode&&pub.phase!=='finished')return renderProjector(pub);
    ...
    const maxRounds=Number(pub.max_rounds||8);
    ...
    screen.innerHTML=`<div class="command-room government-room">
      <header class="command-top government-top">
        ...
        <div class="gov-round"><small>RODADA ${pub.round} / ${maxRounds}</small><b>${esc(phaseTitle(pub))}</b><div class="round-track">${Array.from({length:maxRounds},(_,i)=>`<i class="${i<pub.round?'done':i===pub.round-1?'active':''}"></i>`).join('')}</div></div>
  ...
  function renderGovernmentPost(pub,isTeacher){
    ...
    if(!item)return'';
    const postId=String(item.id||item.at||'');
    if(postId&&state.dismissedGovernmentPostId===postId)return'';
    ...
    return `<aside class="government-post" role="status" aria-live="polite" data-dismiss-government-post="${esc(postId)}" title="Clique para fechar"><div><span>◉ NOTA OFICIAL</span><b>${esc(account)}</b><button type="button" aria-label="Fechar Nota Oficial" data-dismiss-government-post="${esc(postId)}">×</button></div><p>${esc(text||'Novo movimento no cenário internacional.')}</p><small>Atualização temporária · clique para fechar</small></aside>`;
  }
  ...
  function renderCommandEvent(e,pub,isTeacher=false){
    ...
    return `<div class="panel-title"><span>◎</span><div><b>CENÁRIO GLOBAL</b><small>Eventos que moldam o amanhã</small></div></div><article class="event-paper ${density}"><div class="event-paper-top"><span>EVENTO DA RODADA</span><b>${esc(kind)}</b></div><h2>${esc(e.name)}</h2><div class="event-visual"><span>🌐</span><small>BOLETIM INTERNACIONAL · RODADA ${pub.round}</small></div><p class="event-summary">${esc(summary)}</p><div class="event-effect-box"><b>EFEITO NO JOGO</b><p>${esc(e.effect)}</p></div><blockquote>${quote}<cite>— Observatório Político Internacional</cite></blockquote></article>${post}`;
  }
  ...
  function renderTeacherControlCompact(pub){
    ...
    return `<div class="command-section-label">Controle docente</div><h2>Sala ${esc(state.room.code)}</h2><div class="teacher-phase"><b>${esc(phaseTitle(pub))}</b><span>Rodada ${pub.round}/${Number(pub.max_rounds||8)} · ${esc(pub.match_mode_label||'Completa')}</span></div><div class="teacher-controls">${controls||'<span class="muted small">Aguardando ação dos estudantes.</span>'}<button class="btn projector" id="openProjector">Modo Projetor</button><button class="btn danger" id="interruptSession">Interromper sessão</button></div><div class="teacher-hint">Sem cronômetro automático. O professor controla o ritmo e pode encerrar turnos ou fases.</div>`;
  }
  ...
  function openCommandModal(kind,pub){
    ...
    body.innerHTML=body.innerHTML.replace(' / 8</b>',' / '+maxRounds+'</b>').replace('8ª rodada',maxRounds+'ª rodada').replace('regra 0.4-C','regra '+RULES_VERSION);
  ...
  function renderPending(p,isTeacher,own){
    ...
    if(p.kind==='card_setup'){
      const id=Number(p.card_id),stage=p.stage;let buttons='';
      ...
      return `<section class="command-action compact-decision-panel card-setup-panel"><div class="state-kicker">DOSSIÊ EM ANÁLISE</div><h3>Escolha como o governo executará esta decisão</h3>${decisionCardSummary(id)}<div class="actions decision-actions card-setup-actions">${buttons||'<span class="muted">Nenhuma opção válida.</span>'}</div></section>`;
    }
    if(['reaction_loss','reaction_dissolve','reaction_relation_state'].includes(p.kind)){const id=Number(priv.cardId||0);return `<section class="card action-panel reaction-panel compact-decision-panel"><div class="eyebrow">Reação disponível</div><h3>Você pode responder agora</h3>${id?decisionCardSummary(id):''}<div class="actions decision-actions"><button class="btn violet" data-pending="use">Usar Reação</button><button class="btn ghost" data-pending="decline">Não reagir</button></div></section>`}
    ...
    if(p.kind==='trade_response'){const offered=Number(priv.offerCard||0);return `<section class="card action-panel compact-decision-panel"><div class="eyebrow">Troca Comercial</div><h3>${esc(priv.proposer||p.actor)} oferece uma carta</h3>${offered?decisionCardSummary(offered):''}<p class="decision-note">Se aceitar, você escolherá uma carta para entregar. Depois, ambos renovam 1 carta.</p><div class="actions decision-actions"><button class="btn good" data-pending="accept">Aceitar</button><button class="btn danger" data-pending="decline">Recusar</button></div></section>`}
    if(p.kind==='trade_return')return `<section class="card action-panel card-choice-panel"><div class="eyebrow">Troca Comercial</div><h3>Escolha a carta que você entregará</h3>${choiceHandHtml(own?.hand||[],{button:'Entregar esta carta'})}</section>`;
    if(['safeguard_cost','safeguard_block_cost'].includes(p.kind))return `<section class="card action-panel card-choice-panel"><div class="eyebrow">Cláusula de Salvaguarda</div><h3>Escolha 1 carta para descartar como custo</h3>${choiceHandHtml(own?.hand||[],{button:'Descartar como custo'})}</section>`;
    ...
    if(p.kind==='top_two_keep'){const ids=priv.candidates||[];return `<section class="card action-panel card-choice-panel"><div class="eyebrow">Conectividade Global</div><h3>Escolha a carta que ficará com você</h3>${choiceHandHtml(ids,{button:'Ficar com esta'})}</section>`}
    if(p.kind==='replace_unplayable')return `<section class="card action-panel card-choice-panel"><div class="eyebrow">Substituição estratégica</div><h3>Escolha o Dossiê sem alvo que será substituído</h3><p class="flow-explain">A carta escolhida será descartada, você comprará 1 substituta e sua Ação Principal será encerrada.</p>${choiceHandHtml(own?.hand||[],{button:'Substituir esta carta'})}</section>`;
    ...
    if(p.kind==='renew_discard'){const challengeReward=String(p.reason||'').includes('Acerto do líder no Desafio');return `<section class="card action-panel flow-panel card-choice-panel ${challengeReward?'challenge-reward-flow':''}"><div class="eyebrow">${challengeReward?'Etapa 1 de 2 · Recompensa do Desafio':'Renovação de carta'}</div><h3>${challengeReward?'Você acertou como líder: renove 1 carta':esc(p.reason||'Renovação')}</h3>${challengeReward?'<p class="flow-explain">Esta renovação é a recompensa do Desafio e <b>não gasta Vantagem Geográfica</b>. Escolha a carta que será descartada; depois haverá a compra normal da rodada.</p>':''}${choiceHandHtml(own?.hand||[],{button:'Descartar esta carta'})}</section>`}
    if(['discard_card','hand_limit_discard'].includes(p.kind))return `<section class="card action-panel card-choice-panel"><div class="eyebrow">Escolha uma carta</div><h3>${esc(p.reason||'Descarte')}</h3>${choiceHandHtml(own?.hand||[],{button:'Descartar'})}</section>`;
  ...
  function eventChoicePanel(p){
    ...
    if(eid===7)html=`<button class="btn danger" data-pending="lose">Perder 1 Economia</button>${(p.relations||[]).map(r=>`<button class="btn warn" data-pending="tension:${r.partner}">Colocar Acordo com ${esc(r.partner)} sob Tensão</button>`).join('')}`;
    if(eid===8)html=`<button class="btn danger" data-pending="lose">Perder 1 Economia</button>${(p.relations||[]).map(r=>`<button class="btn warn" data-pending="suspend:${r.partner}">Suspender Acordo com ${esc(r.partner)}</button>`).join('')}`;
  ...
  function renderTurn(pub,isTeacher){
    ...
    const c=pub.countries[state.me.country],zeros=Object.keys(ATTRS).filter(a=>Number(c[a])===0),hand=ownPrivate()?.hand||[];
    const playable=hand.filter(id=>!REACTION_IDS_UI.has(Number(id))&&cardHasValidTarget(Number(id),pub,state.me.country));
    const replacement=!playable.length&&hand.length?'<button class="cabinet-action replace" data-turn-action="replace_unplayable"><span>↻</span><b>SUBSTITUIR DOSSIÊ SEM ALVO</b><small>Descarte 1, compre 1 e encerre o turno</small></button>':'';
    return `<section class="command-action presidential-turn active"><div class="state-kicker">SUA AÇÃO PRINCIPAL</div><h1>O que o governo fará?</h1><p>${replacement?'Nenhum Dossiê da mão possui alvo válido. Você pode substituí-lo sem aumentar o tamanho da mão.':'Abra um Dossiê na faixa inferior para analisar a ação antes de confirmá-la. A Diplomacia formal acontece na Cúpula ao fim da rodada.'}</p><div class="presidential-actions">${zeros.map(a=>`<button class="cabinet-action recover" data-recovery="${a}"><span>⚙</span><b>RECUPERAÇÃO NACIONAL</b><small>${ATTRS[a]} · 0 → 1</small></button>`).join('')}${replacement}<button class="cabinet-action pass" data-turn-action="pass"><span>»</span><b>PASSAR TURNO</b><small>Encerrar sem usar um Dossiê</small></button></div></section>`;
  }

  function summitAdvice(own,unrelated,blockTargets,tradeTargets){
    const hand=(own?.hand||[]).map(id=>CARD[id]).filter(Boolean);
    const agreementCards=hand.filter(c=>(c.tags||[]).includes('Acordo')).length;
    const blockCards=hand.filter(c=>(c.tags||[]).includes('Bloco')).length;
    const tips=[];
    if(unrelated.length&&agreementCards)tips.push(`<span><b>Acordo:</b> ${agreementCards} Dossiê${agreementCards===1?'':'s'} em sua mão pode${agreementCards===1?'':'m'} se beneficiar dessa relação.</span>`);
    if(blockTargets.length&&blockCards)tips.push(`<span><b>Bloco:</b> ${blockCards} Dossiê${blockCards===1?'':'s'} em sua mão usa${blockCards===1?'':'m'} integração econômica.</span>`);
    if(tradeTargets.length&&hand.length)tips.push('<span><b>Troca:</b> consulte o efeito antes de oferecer um Dossiê que não combina com sua estratégia.</span>');
    if(!tips.length)tips.push('<span>Nenhuma oportunidade diplomática evidente foi identificada. Passar continua sendo uma decisão válida.</span>');
    return `<div class="summit-advice"><b>LEITURA DO CONSELHO</b>${tips.slice(0,2).join('')}</div>`;
  }

  function renderDiplomacyPhase(pub,isTeacher,own){
    const active=activeList(pub),done=pub.diplomacy_done||[],order=(pub.diplomacy_order?.length?pub.diplomacy_order:active),current=pub.active_country;
    const statusGrid=order.map(c=>`<div class="dip-status ${done.includes(c)?'done':current===c?'current':''}"><b>${esc(c)}</b><span>${done.includes(c)?'✓ concluída':current===c?'◆ iniciativa atual':'aguardando'}</span></div>`).join('');
    if(isTeacher)return `<section class="command-action summit-state"><div class="state-kicker">CÚPULA INTERNACIONAL · ORDEM DAS INICIATIVAS</div><h1>${current?`${esc(current)} está com a palavra`:'Todas as iniciativas foram concluídas'}</h1><div class="diplomacy-status-grid">${statusGrid}</div><p class="muted tiny">A ordem acompanha os turnos nacionais desta rodada. Respostas a propostas não consomem a iniciativa do país-alvo.</p></section>`;
    ...
    if(done.includes(me))return `<section class="command-action summit-state"><div class="state-kicker">CÚPULA INTERNACIONAL</div><h1>Sua iniciativa foi concluída</h1><p>Você ainda pode receber e responder propostas quando outro governo estiver com a palavra.</p><div class="diplomacy-status-grid">${statusGrid}</div></section>`;
    ...
    if(current&&current!==me)return `<section class="command-action summit-state"><div class="state-kicker">CÚPULA INTERNACIONAL · ORDEM DAS INICIATIVAS</div><h1>Aguardando ${esc(current)}</h1><p>Sua delegação terá a palavra quando chegar sua posição na ordem desta rodada.</p><div class="diplomacy-status-grid">${statusGrid}</div></section>`;
    const repairTargets=Object.entries(pub.relations||{}).filter(([k,r])=>k.split('|').includes(me)&&(r.tension||r.suspended||r.crisis)).map(([k,r])=>({partner:k.split('|').find(x=>x!==me),state:r.crisis?'Crise':r.suspended?'Suspensão':'Tensão',type:r.type}));
    const repairBox=repairTargets.length?`<div class="summit-repair"><b>REPARAR RELAÇÃO</b><span>Consome sua iniciativa. Um segundo abalo antes do reparo dissolve a relação.</span><div>${repairTargets.map(x=>`<button class="btn warn compact" data-diplomacy="repair_relation" data-target="${esc(x.partner)}">${x.type==='block'?'Restabelecer Bloco':'Normalizar Acordo'} com ${esc(x.partner)} · ${x.state}</button>`).join('')}</div></div>`:'';
    const secondary=summitAdvice(own,unrelated,blockTargets,tradeTargets)+((agreements.length||block)?`<div class="summit-secondary">${agreements.length?`<div class="summit-secondary-action danger"><b>ENCERRAR ACORDO</b><span>${agreements.map(t=>`<button data-diplomacy="end_agreement" data-target="${esc(t)}">Com ${esc(t)}</button>`).join('')}</span></div>`:''}${block?`<div class="summit-secondary-action danger"><b>SAIR DO BLOCO · −1 Diplomacia</b><span><button data-diplomacy="exit_block">Com ${esc(block)}</button></span></div>`:''}</div>`:'');
    return `<section class="command-action summit-state"><div class="summit-heading"><div><div class="state-kicker">CÚPULA INTERNACIONAL · SUA VEZ</div><h1>Escolha a iniciativa de ${esc(me)}</h1></div><strong>1 iniciativa disponível</strong></div><div class="summit-flags">${repairBox}${openingBonus?'<div class="summit-bonus"><b>Abertura Comercial</b><span>Novo Acordo → ambos renovam 1 carta.</span></div>':''}${extraBox}</div><div class="summit-actions summit-primary"><div class="summit-box"><b>PROPOR ACORDO</b><small>Crie uma Relação Comercial.</small><div class="summit-target-pills">${unrelated.map(t=>`<button data-diplomacy="agreement" data-target="${esc(t)}">${esc(t)}</button>`).join('')||'<em>Sem alvo válido</em>'}</div></div><div class="summit-box"><b>PROPOR BLOCO</b><small>Limite: 1 Bloco por país.</small><div class="summit-target-pills">${blockTargets.map(t=>`<button data-diplomacy="block" data-target="${esc(t)}">${esc(t)}</button>`).join('')||'<em>Sem alvo válido</em>'}</div></div><div class="summit-box trade"><b>TROCAR CARTA</b><small>Escolha o país e o Dossiê oferecido.</small><select id="tradeTarget" aria-label="País para troca">${tradeTargets.map(t=>`<option value="${esc(t)}">${esc(t)}</option>`).join('')}</select><select id="tradeCard" aria-label="Carta oferecida">${(own?.hand||[]).map(id=>`<option value="${id}">${esc(CARD[id]?.name||'Carta '+id)}</option>`).join('')}</select><div class="summit-trade-actions"><button id="tradePreviewBtn" type="button" ${!(own?.hand||[]).length?'disabled':''}>Ver efeito</button><button id="tradeSubmit" ${(!tradeTargets.length||!(own?.hand||[]).length)?'disabled':''}>Propor troca</button></div></div><div class="summit-box pass"><b>NÃO REALIZAR AÇÃO</b><small>Conclua a iniciativa sem alterar relações.</small><button data-diplomacy="pass">Encerrar iniciativa</button></div></div>${secondary}</section>`;
  }
  ...
  function bindMatchActions(pub,isTeacher,own){
    ...
    document.querySelector('[data-turn-action="replace_unplayable"]')?.addEventListener('click',()=>withBusy(async()=>{await api('turn_action',{roomId:state.room.id,kind:'replace_unplayable'});await refreshSnapshot()}));
    ...
    document.querySelectorAll('[data-dismiss-government-post]').forEach(b=>b.onclick=e=>{e.stopPropagation();state.dismissedGovernmentPostId=String(b.dataset.dismissGovernmentPost||'');renderRoom()});
    ...
    if(isTeacher){
      document.getElementById('openProjector')?.addEventListener('click',()=>{state.projectorMode=true;renderRoom()});
  ...
  function renderProjector(pub){
    const maxRounds=Number(pub.max_rounds||8),active=activeList(pub),answered=(pub.challenge_answered||[]).length,done=(pub.diplomacy_done||[]).length;
    const relations=Object.entries(pub.relations||{});
    const feed=(pub.bulletins||[]).slice(-6).reverse();
    const phaseProgress=pub.phase==='challenge'?`${answered}/${active.length} países responderam`:pub.phase==='diplomacy'?`${done}/${active.length} iniciativas concluídas${pub.active_country?` · Em foco: ${pub.active_country}`:''}`:pub.active_country?`Em foco: ${pub.active_country}`:'Acompanhamento público';
    document.body.classList.add('in-match','projector-active');
    screen.innerHTML=`<div class="projector-view"><header><div><span>GEOPODER · PAINEL PÚBLICO</span><h1>${esc(state.room.class_name||state.room.code)}</h1></div><div class="projector-round"><b>RODADA ${pub.round} / ${maxRounds}</b><span>${esc(phaseTitle(pub))}</span></div><button class="btn ghost" id="closeProjector">Voltar ao controle</button></header><section class="projector-event ${eventTone(pub.current_event)}"><div><small>EVENTO DA RODADA</small><h2>${esc(pub.current_event?.name||'Aguardando Evento Global')}</h2><p>${esc(pub.current_event?.effect||'A partida ainda não revelou o próximo acontecimento.')}</p></div><strong>${esc(phaseProgress)}</strong></section><section class="projector-countries">${active.map(c=>{const d=pub.countries?.[c]||{},total=Number(d.eco||0)+Number(d.net||0)+Number(d.dip||0)+Number(d.cult||0);return `<article class="country-${esc(c)}"><div>${countrySeal(c)}<h3>${esc(c)}</h3></div><strong>${total}<small>Influência</small></strong><ul><li>Economia <b>${d.eco??0}</b></li><li>Redes <b>${d.net??0}</b></li><li>Diplomacia <b>${d.dip??0}</b></li><li>Cultura <b>${d.cult??0}</b></li></ul></article>`}).join('')}</section><section class="projector-lower"><article><small>RELAÇÕES INTERNACIONAIS</small><div class="projector-relations">${relations.length?relations.map(([k,r])=>{const[a,b]=k.split('|'),stateLabel=r.suspended?'Suspenso':r.tension?'Sob tensão':r.crisis?'Em crise':'Ativo';return `<div><b>${esc(a)} ↔ ${esc(b)}</b><span>${r.type==='block'?'Bloco':'Acordo'} · ${stateLabel}</span></div>`}).join(''):'<p>Nenhuma relação formal foi criada.</p>'}</div></article><article><small>ÚLTIMOS BOLETINS</small><div class="projector-feed">${feed.length?feed.map(b=>`<div><b>${esc(b.title||'Atualização')}</b><span>${esc(b.body||'')}</span></div>`).join(''):'<p>Aguardando decisões dos governos.</p>'}</div></article></section></div>`;
    document.getElementById('closeProjector').onclick=()=>{state.projectorMode=false;document.body.classList.remove('projector-active');renderRoom()};
  }
  ...
  function buildMarkdown(d){
    const teams=(d.players||[]).filter(p=>p.role==='player'),events=d.events||[],counts=countEvents(events),q=questionStats(events),c=cardStats(events),timing=turnTimingStats(events),dip=diplomacyStats(events),lab=labStats(events),notes=events.filter(e=>e.event_type==='TEACHER_NOTE');
