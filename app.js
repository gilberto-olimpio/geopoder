(() => {
  ...
  const GAME_VERSION = 'Alpha 2.0b.1';
  ...
  let sb = null;
  let state = {session:null,teacherProfile:null,room:null,me:null,players:[],match:null,privateStates:[],channel:null,busy:false,dashboard:null,historyDetail:null,resumeSnapshot:null,interrupting:false};
  ...
  async function refreshSnapshot(){if(!state.room?.id)return;try{const s=await api('snapshot',{roomId:state.room.id});state.room=s.room;state.me=s.me;state.players=s.players||[];state.match=s.match;state.privateStates=s.privateStates||[];renderRoom()}catch(e){console.warn(e)}}
