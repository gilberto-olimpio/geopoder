(() => {
  ...
  const GAME_VERSION = 'Alpha 3.0a';
  const RULES_VERSION = '0.6-DN';
  ...
  function labStats(events){
    ...
    return{started,recoveries,turnPasses,turns:turns.length,summits:summitRounds.size,summitsNoProposal,medianAnswer};
  }
  ...
  function renderProjector(pub){
    ...
    const phaseProgress=pub.phase==='challenge'?`${answered}/${active.length} países responderam`:pub.phase==='diplomacy'?`${done}/${active.length} iniciativas concluídas`:pub.active_country?`Em foco: ${pub.active_country}`:'Acompanhamento público';
